...
namespace Rosetta.Lib.Functions
{
    ...
    public static class EqualsExtension
    {
        ...
        public static bool Equals<T>(this IEnumerable<T>? collection, T? obj)
        {
            return collection?.All(c => c != null && c.Equals(obj)) == true;
        }

        public static bool NotEquals<T>(this IEnumerable<T>? collection, T? obj) => collection?.Equals(obj) != true;
    ...
    {
        // generic comparisons
        private static bool Compare<T>(IEnumerable<T>? collection, T? obj, Func<int, bool> func) where T : IComparable
        {
            return collection?.All(c => func(c.CompareTo(obj))) == true;
        }

        public static bool GreaterThan<T>(this IEnumerable<T>? collection, T? obj) where T : IComparable => Compare(collection, obj, i => i > 0);
        public static bool GreaterThanEquals<T>(this IEnumerable<T> collection, T obj) where T : IComparable => Compare(collection, obj, i => i >= 0);
        public static bool LessThan<T>(this IEnumerable<T> collection, T obj) where T : IComparable => Compare(collection, obj, i => i < 0);
        public static bool LessThanEquals<T>(this IEnumerable<T> collection, T obj) where T : IComparable => Compare(collection, obj, i => i <= 0);
