...
import static extension com.regnosys.rosetta.generator.jsonschema.JsonSchemaTranslator.*
...
class JsonSchemaMetaFieldGenerator {
	...
	private def generateAttribute(ExpandedType type) {
		...
		@JsonJsonSchemaEnumeration(classOf[«type.name».Class])
		value: Option[«type.toJsonSchemaType»]'''
		} else {
			'''value: Option[«type.toJsonSchemaType»]'''
		}
	}
	...
	private def generateReferenceWithMeta(ExpandedType type) '''
		case class ReferenceWithMeta«type.toMetaTypeName»(value: Option[«type.toJsonSchemaType»],
	...
	private def generateBasicReferenceWithMeta(ExpandedType type) '''
		case class BasicReferenceWithMeta«type.toMetaTypeName»(value: Option[«type.toJsonSchemaType»],
				globalReference: Option[String],
				externalReference: Option[String],
				address: Option[Reference]) {}
	...
	private def genMetaFields(Iterable<RosettaMetaType> types, String version) '''				
		case class MetaFields(«FOR type : types.distinctBy(t|t.name.toFirstLower) SEPARATOR '\n		'»«type.name.toFirstLower»: Option[«type.typeCall.type.name.toJsonSchemaBasicType»],«ENDFOR»
