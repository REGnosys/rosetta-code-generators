...
def test_attribute_single ():
	assert rosetta_resolve_attr(foo, 'one') == 42
def test_attribute_optional ():
	assert rosetta_resolve_attr(foo, 'two') is None
def test_attribute_multi ():
	assert rosetta_resolve_attr(foo, 'three') == [1, 2, 3]
def test_attribute_single_collection (): 
	assert rosetta_resolve_attr([foo, foo], 'one') == [42, 42]
def test_attribute_optional_collection (): 
	assert rosetta_resolve_attr([foo, foo], 'two') is None
def test_attribute_multi_collection ():
	assert rosetta_resolve_attr([foo, foo], 'three') == [1, 2, 3, 1, 2, 3]
