...
class PythonFilesGeneratorTest {
	...
	def void generatePython() {
		...
		try {
			val iniFileName = new File("src/main/resources/runtime/pythonCDMGenerator.ini")
			...
			var dslFile = Path.of(dslPath)
			var listFiles = Files.list(dslFile)
				.filter[it.toString.endsWith('.rosetta')]
				.map[toFile]
			...
			println(
				'PythonFilesGeneratorTest::generatePython ... found ' + listFiles.length.toString() +
					' rosetta files in: ' + dslPath)
			...
			createStrucurePython(resourcesPath, dslPath, cdmVersion, pythonTgtPath, listFiles, resourceSet,
				importsVariables)
...
//			val existingCreatedFunctions = new File(resourcesPath + File.separator + "functions").list().map [
//				it.split("\\.").get(0)
//			]
			...
			for (RosettaRootElement element : model.elements) {
				...
				if (element instanceof RosettaEnumeration || element instanceof Data || element instanceof Function) {
					...
					val python = null; //generator.afterGenerate(Collections.singletonList(model), element, importsVariables)
					...
					println("namespace " + model.name + " topLevelKey " + topLevelKey + " secondLevelKey " + secondLevelKey + " lastLevelKey " + lastLevelKey)
					...
					for (String key : parseModelName) {
						...
						if (key == topLevelKey && !seenRoot) {
							...
							if (key == secondLevelKey && !dirFileCount.containsKey(key)) {
								...
								println(
									'PythonFilesGeneratorTest::createStrucurePython ... removed and created directory: ' +
										directory + ' key: ' + key)
								dirFileCount.put(key, 0)
							}
						...
						if (key == lastLevelKey) {
							...
							val funcExists = false //existingCreatedFunctions.contains(className)
							...
							if (containsKey) {
								if (isFunc && funcExists) {
									Files.copy(
										Paths.get(
											resourcesPath + File.separator + "functions" + File.separator + className +
												".py"),
		...
		for (String key : dirFileCount.keySet()) {
			println('PythonFilesGeneratorTest::createStrucurePython ... created dir: ' + key + ' with # files: ' +
				dirFileCount.get(key))
		}
	}
	 ...
	 */
	def private HashMap<String, List<String>> loadResourceSet(File[] listFiles, ResourceSet resourceSet,
		HashMap<String, List<String>> importsVariables) {
